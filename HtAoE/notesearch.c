...
int find_user_note(int,int); // Seek in file a note for user
...
int main(int argc, char **argv) {
  ...
  if(argc > 1) // If there is an argument
    strcpy(searchstring,argv[1]); // it is the search string
  else
...
int find_user_note(int fd,int user_uid) {
  ...
  int note_uid = -1, length;
  unsigned char byte;

  while(note_uid != user_uid) { // Loop until a note for user_uid is found
    if(read(fd,&note_uid,4) != 4) // Read the uid data
      return(-1); // If 4 bytes aren't read return the end of file code
	...
	return(-1); // If byte isn't read return the end of file code
      length++;
    }
  }
